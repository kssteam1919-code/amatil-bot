...
async def handle_all(message: types.Message):
    ...
    if message.text and message.text.startswith("/confirm"):
        ...
        if len(parts) > 1:
            ...
            if order_id in pending_orders:
                ...
                await bot.send_message(MY_ID, f"✅ Заказ {order_id} подтверждён! Товары списаны.")
                await message.answer(f"✅ Заказ {order_id} подтверждён!")
            else:
                await message.answer("❌ Заказ не найден.")
        else:
            await message.answer("❌ Укажите ID заказа: /confirm [id]")
        return
    ...
    if message.web_app_data:
        ...
        pending_orders[order_id] = {"items": order, "total": total, "user": name, "userId": user.id}
        save_json(PENDING_FILE, pending_orders)
        ...
        await message.answer("✅ Заказ отправлен! Ожидайте подтверждения.\n📲 @Fe3riko")
        return
    ...
    if message.text == "💬 Отзывы":
        ...
        facts = ["Первый прототип вейпа — 1965 год.", "Качественная жижа не содержит диацетил.", "Мятные вкусы — самые популярные."]
        await message.answer(f"🧠 {random.choice(facts)}")
    elif message.text == "🎰 Крутить скидку":
        if user_id in user_wheel_used:
            await message.answer("🎰 Вы уже крутили сегодня. Приходите завтра!")
        else:
            prize = random.choice(PRIZES)
            user_wheel_used[user_id] = True
            await message.answer(f"🎰 Ваш приз: {prize['prize']}\nПромокод: {prize['code']}\nПредъявите @Fe3riko")
