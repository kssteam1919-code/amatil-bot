...
async def handle_all(message: types.Message):
    ...
    if user_id in user_game_state and text == "🚪 Выйти из игры":
        ...
        await message.answer("🚪 Вы вышли из игры. Можете продолжить пользоваться ботом.", reply_markup=get_main_keyboard())
        return
    ...
    if message.web_app_data:
        ...
        for item in order:
            text_msg += f"• {item['name']} × {item['qty']} — {item['price'] * item['qty']} ₽\n"
        text_msg += f"\n💰 Итого: {total} ₽\n\n⚡ /confirm {order_id}"
