...
PRIZES = [
    ...
    {"prize": "Скидка 10%", "code": "WHEEL10"}, {"prize": "Скидка 10%", "code": "WHEEL10"},
    {"prize": "Скидка 10%", "code": "WHEEL10"}, {"prize": "Скидка 10%", "code": "WHEEL10"},
    {"prize": "Скидка 10%", "code": "WHEEL10"}, {"prize": "Скидка 15%", "code": 
...
async def handle_all(message: types.Message):
    ...
    if user_id in user_game_state and message.text == "🚪 Выйти из игры":
        ...
        await message.answer("🚪 Вы вышли из игры «Угадай вкус». Можете продолжить пользоваться ботом. /start")
        return
    ...
    if message.text == "💬 мнение наших покупателей":
        await message.answer("💬 🔍 Сомневаетесь в выборе?

Не верьте нам на слово — послушайте тех, кто уже сделал заказ. В нашей группе отзывов собраны реальные мнения реальных парильщиков.

👇 Переходите по ссылке, листайте и убедитесь сами. Если у вас есть что добавить — будем только рады! — @Fe3riko")
    elif message.text == "📞 Поддержка":
        await message.answer("📞 @Fe3riko\n\📞 Есть вопросы?

Сломалась любимая жижа? Не можете выбрать подик? Или просто скучно?

Наши консультанты уже заточили пальцы для быстрого набора текста. Не стесняйтесь — мы не кусаемся, только помогаем 💪.")
    elif message.text == "❓ FAQ":
        await message.answer("🚗 Доставка по городу\n💳 Оплата перевод/наличные\n🕒 24/7")
    elif message.text == "🎁 Акции":
        await message.answer("👥 Приведи друга — скидка 10%\n🎁 Заказ от 2000₽ — подарок\n🎰 Крути колесо!")
