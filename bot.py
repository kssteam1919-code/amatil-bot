...
# Для мини-игры "Угадай вкус"
FLAVORS = {
    "вишня": "Скидка 10% — WHEEL10",
    "мята": "Скидка 15% — WHEEL15",
    "ананас": "Скидка 5% — WHEEL5",
    "грейпфрут": "Бесплатная доставка — FREEDEL",
    "банан": "Скидка 20% — WHEEL20",
    "яблоко": "Скидка 10% — WHEEL10",
    "персик": "Скидка 5% — WHEEL5",
    "гранат": "Скидка 15% — WHEEL15"
}

current_stock = {
    "rickmorty": 1, "monk": 1, "monk2": 1, "samoubiyca_v2": 2,
    "monstervapor": 2, "annima_apple": 0,
    "h45_green": 1, "pasito2": 1, "pasito3": 1, "hamster1": 0
}

pending_orders = {}
user_wheel_used = {}
subscribers = set()  # для уведомлений о новинках
referrals = {}      # для реферальной системы
total_revenue = 0   # для подсчёта выручки

async def handle(request):
    return web.Response(text="OK")

# Автосброс колеса фортуны каждые 24 часа
...
async def handle_all(message: types.Message):
    ...
    if message.text and message.text.startswith("/confirm"):
        ...
        if len(parts) > 1:
            ...
            if order_id in pending_orders:
                ...
                await message.answer(f"❌ Заказ не найден.")
        else:
    ...
    elif message.text == "🎮 Угадай вкус":
        flavor = random.choice(list(FLAVORS.keys()))
        await message.answer(f"🎮 Угадайте вкус дня!\n\nПодсказка: это фрукт или ягода.\n\nНапишите свой вариант!")
        # Сохраняем загаданный вкус
        global current_flavor
        current_flavor = flavor
    elif message.text and message.text.lower() in FLAVORS:
        if message.text.lower() == current_flavor:
            prize = FLAVORS[message.text.lower()]
            await message.answer(f"🎉 Верно! Вы выиграли: {prize}")
        else:
            await message.answer("❌ Не угадали! Попробуйте ещё раз.")
